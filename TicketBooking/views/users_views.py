...
def user(request):
    ...
    if request.method=="POST":
        ...
        users=User.objects.raw("select * from ticketbooking_user limit 3 offset %s",[offset])
    else:
        users=User.objects.raw("select * from ticketbooking_user limit 3 offset 0")
    return render (request,"dashboard/users.html",{'users':users, 'page': page})
